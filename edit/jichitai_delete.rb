...
begin
    ...
    if delete=='true' && c_id!=''
        ...
        my.prepare("UPDATE jichitai SET note=? WHERE j_id=?;").execute(note, c_id)
        my.close()

    elsif delete!='true' && c_id!=''
        h_t = '削除しますか？'
        confirm = "<a href="+"/app/edit/jichitai_delete.rb?delete=true"+'&note='+note+'&c_id='+c_id+">id="+c_id+" 削除</a>"
    else
        h_t = '削除失敗'
    end
    print <<-EOF
